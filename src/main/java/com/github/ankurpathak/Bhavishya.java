 ...
 *
 */
public class Bhavishya
{

    static Logger logger = LoggerFactory.getLogger(Bhavishya.class);
    private static String template = "enetFile.xls";
    private static String xmlConfig = "enetFile.xml";
    private static String output = "target/enetFile_output.xls";
    public static void main( String[] args ) throws Exception
    {
        logger.info("Opening input stream");
        try(InputStream is = Bhavishya.class.getResourceAsStream(template)) {
            try (OutputStream os = new FileOutputStream(output)) {
                Transformer transformer = TransformerFactory.createTransformer(is, os);
                System.out.println("Creating areas");
                InputStream configInputStream = Bhavishya.class.getResourceAsStream(xmlConfig);
